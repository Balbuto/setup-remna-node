#!/bin/bash
# ==============================================================================
# 🚀 ИНТЕРАКТИВНЫЙ ИНСТРУМЕНТ НАСТРОЙКИ И УПРАВЛЕНИЯ REMNANODE (v1.3.2)
# ==============================================================================
...
generate_masked_template() {
    ...
    # 3. Применение мутации к HTML файлам
    find "$WWW_DIR" -type f -name "*.html" -exec sed -i \
        ...
        -e "s/fonts\.googleapis\.com/d; s/fonts\.gstatic\.com/d" \
        {} \;
...
EOF
    ...
    # Формирование docker-compose.yml
    if [ "$protocol" = "reality" ]; then
        # === Сценарий VLESS Reality (Selfsteal + Nginx через Unix Socket) ===
...
EOF
        ...
        # === Сценарий VLESS+TLS (SSL сертификаты в remnanode секции) ===
        cat > docker-compose.yml <<EOF
services:
  remnanode:
    image: ghcr.io/remnawave/node:latest
    ...
    volumes:
      - /dev/shm:/dev/shm:rw
      - $LOG_DIR/node:/var/log/remnanode
      - $CERTS_DIR:/etc/xray/certs:ro # SSL-сертификаты примонтированы в контейнер
$( [ "$XRAY_VERSION_CHOICE" != "built-in" ] && echo "      - $XRAY_FILE:/usr/local/bin/xray" )
$( [ "$XRAY_VERSION_CHOICE" != "built-in" ] && [ -f "$XRAY_BIN_DIR/geoip.dat" ] && echo "      - $XRAY_BIN_DIR/geoip.dat:/usr/local/share/xray/geoip.dat" )
$( [ "$XRAY_VERSION_CHOICE" != "built-in" ] && [ -f "$XRAY_BIN_DIR/geosite.dat" ] && echo "      - $XRAY_BIN_DIR/geosite.dat:/usr/local/share/xray/geosite.dat" )
EOF
...
configure_firewall() {
    ...
    # 4. Строгое ограничение: порт управления доступен ТОЛЬКО для IP панели
...
run_initial_setup() {
    ...
    # Выпуск SSL или запуск маскировки
    if [ "$protocol" = "reality" ]; then
...
change_domain_and_ssl() {
    ...
    if [ "$protocol" = "tls" ]; then
        ...
        # Для Reality просто перегенерируем маскировку
        read -p "Домен маскировки (decoy domain) [github.com]: " decoy_domain
        DECOY_DOMAIN=${decoy_domain:-github.com}
        generate_masked_template "$DECOY_DOMAIN"
    fi
...
uninstall_and_rollback() {
    ...
    echo -e "${RED}логи, маскировочные сайты и вернет брандмауэр UFW/системные оптимизации в исходное состояние.${NC}"
    echo -e "${GRAY}--------------------------------------------------${NC}"
    ...
    echo -e "\n${GREEN}========================================================${NC}"
    echo -e "${SUCCESS} Нода Remnanode успешно удалена с сервера!"
...
main_menu() {
    while true; do
        ...
        echo -e "${GREEN}"
        echo "  ========================================================"
        echo "  🚀 REMNANODE ИНТЕРАКТИВНОЕ УПРАВЛЕНИЕ НОДОЙ (v1.3.2)"
        echo "  ========================================================"
        echo -e "${NC}"
        ...
        echo " 0) Выход"
        echo -e "${GRAY}--------------------------------------------------${NC}"
        read -p "Выберите действие [0-6]: " menu_choice
