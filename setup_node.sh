#!/bin/bash
# ==============================================================================
# 🚀 ИНТЕРАКТИВНЫЙ ИНСТРУМЕНТ НАСТРОЙКИ И УПРАВЛЕНИЯ REMNANODE (v1.6.1)
# ==============================================================================
...
EOF
        ...
        # VLESS+TLS / VLESS+TLS+xHTTP: Xray сам расшифровал TLS трафик на внешнем 443 порту с Let's Encrypt,
...
run_initial_setup() {
    ...
    if [ "$protocol" = "reality" ]; then
        ...
        # Для TLS/xHTTP генерируем маскировку под собственный домен ноды (для fallback)
...
main_menu() {
    while true; do
        ...
        echo -e "${GREEN}"
        echo "  ========================================================"
        echo "  🚀 REMNANODE ИНТЕРАКТИВНОЕ УПРАВЛЕНИЕ НОДОЙ (v1.6.1)"
        echo "  ========================================================"
        echo -e "${NC}"
        ...
        echo " 0) Выход"
        echo -e "${GRAY}--------------------------------------------------${NC}"
        read -p "Выберите действие [0-9]: " menu_choice
