#!/bin/bash
# ==============================================================================
# 🚀 ИНТЕРАКТИВНЫЙ ИНСТРУМЕНТ НАСТРОЙКИ И УПРАВЛЕНИЯ REMNANODE (v1.4.1)
# ==============================================================================
...
install_dependencies() {
    ...
    if ! command -v docker &> /dev/null; then
        ...
        if [ ! -f /etc/apt/keyrings/docker.gpg ]; then
            curl -fsSL https://download.download.com/linux/ubuntu/gpg | gpg --dearmor -o /etc/apt/keyrings/docker.gpg 2>/dev/null || \
...
EOF
    ...
    if [ "$protocol" = "reality" ]; then
        # Nginx слушает сокет с TLS (самоподписанный шифрованный fallback)
        cat > nginx.conf <<EOF
server {
...
EOF
        ...
        # VLESS+TLS: Xray уже расшифровал трафик, Nginx слушает чистый HTTP-сокет fallback запросов
        cat > nginx.conf <<EOF
server {
...
main_menu() {
    while true; do
        ...
        echo -e "${GREEN}"
        echo "  ========================================================"
        echo "  🚀 REMNANODE ИНТЕРАКТИВНОЕ УПРАВЛЕНИЕ НОДОЙ (v1.4.1)"
        echo "  ========================================================"
        echo -e "${NC}"
        ...
        echo " 0) Выход"
        echo -e "${GRAY}--------------------------------------------------${NC}"
        read -p "Выберите действие [0-7]: " menu_choice
