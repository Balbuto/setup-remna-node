#!/bin/bash
# ==============================================================================
# 🚀 ИНТЕРАКТИВНЫЙ ИНСТРУМЕНТ НАСТРОЙКИ И УПРАВЛЕНИЯ REMNANODE (v1.7.1)
# ==============================================================================
...
#  - Встроенный модуль комплексной диагностики и мульти-тестов сервера (YABS, Censorcheck)
#  - Исправлена глобальная регистрация команды 'remnanode' при удаленном curl-запуске
...
# --- Ссылка на сырой код скрипта в вашем репозитории (для удаленной регистрации и автообновлений) ---
# ВАЖНО: Если вы форкаете проект, измените этот URL на адрес вашего личного форка!
SCRIPT_URL="https://raw.githubusercontent.com/Mrvibecodic/node-templates/main/setup_node.sh"
...
run_server_multitests() {
    (
        ...
        while true; do
            ...
            echo -e "${GREEN}"
            echo "  ========================================================"
            echo "  🧪 МУЛЬТИ-ТЕСТЫ И ДИАГНОСТИКА СЕРВЕРА (v1.7.1)"
            echo "  ========================================================"
            echo -e "${NC}"
            ...
            case $test_choice in
                ...
                10)
                    ...
                    echo -e "\n${BLUE}Выполнение Ping до yandex.ru:${NC}"
                    ping -c 4 yandex.ru || true
                    echo -e "\n${BLUE}Выполнение Traceroute до yandex.ru:${NC}"
                    traceroute -m 20 yandex.ru || true
                    pause_prompt
                    ;;
                99)
                    clear
                    echo -e "${BLUE}Выполнение ВСЕХ тестов по очереди (Выход: Ctrl+C)...${NC}\n"
                    ...
                    # 3. Censorcheck DPI
                    run_verified_benchmark "Censorcheck (DPI)" "https://github.com/vernette/censorcheck/raw/master/censorcheck.sh" "--mode dpi"
...
main_menu() {
    while true; do
        ...
        echo -e "${GREEN}"
        echo "  ========================================================"
        echo "  🚀 REMNANODE ИНТЕРАКТИВНОЕ УПРАВЛЕНИЕ НОДОЙ (v1.7.1)"
        echo "  ========================================================"
        echo -e "${NC}"
